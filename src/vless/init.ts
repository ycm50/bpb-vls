...
export async function setSettings(request: Request, env: Env) {
    ...
    // VLESS-only: always ensure Trojan is disabled
    dataset.settings.TRConfigs = false;
    globalThis.settings = dataset.settings;
}

export function init(request: Request, env: Env) {
    const { pathname } = new URL(request.url);
    const { UUID, FALLBACK, DOH_URL } = env;
...
export function initHttp(request: Request, env: any) {
    const { _VL_CAP_, _website_ } = globalThis.dict;
    const { UUID, SUB_PATH, kv } = env;
    ...
    if (!['/secrets', '/favicon.ico'].includes(decodeURIComponent(pathname))) {
        if (!UUID) throw new Error(`Please set ${_VL_CAP_} UUID first. Visit <a href="${origin}/secrets" target="_blank">here</a> to generate them.`, { cause: "init" });
