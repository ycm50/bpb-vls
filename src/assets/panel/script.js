...
function populatePanel(proxySettings) {
    const dohElm = document.getElementById("doh");
    if (dohElm) dohElm.textContent = `${window.origin}/dns-query/${decodeURIComponent(globalThis.subPath)}`;
    ...
    textareaElements.forEach(elm => {
        ...
        const arr = proxySettings[key];
        if (!arr) return;
        const value = arr.join('\r\n');
        element.style.height = 'auto';
        if (arr.length) element.rows = arr.length;
        element.value = value;
    });
}
