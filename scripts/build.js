...
/**
 * Build a single worker entry point.
 * @param {string} entryName - Entry file name (e.g. 'worker' or 'vless-worker')
 * @param {'full' | 'vless'} variant - Build variant
 */
async function buildWorkerEntry(entryName, variant = 'full', outName) {
    const label = variant === 'vless' ? 'VLESS-only' : 'Full';
    const htmls = await processHtmlPages();
    const faviconBuffer = readFileSync('./src/assets/favicon.ico');
    const faviconBase64 = faviconBuffer.toString('base64');

    const code = await build({
        entryPoints: [join(__dirname, `../src/${entryName}.ts`)],
        bundle: true,
        format: 'esm',
        write: false,
    ...
    console.log(`${success} ${label} Worker built successfully!`);
    ...
    const minifyCode = async (code) => {
        ...
        console.log(`${success} ${label} Worker minified successfully!`);
        return minified;
    }
    ...
    if (mangleMode) {
        ...
        console.log(`${success} ${label} Worker obfuscated successfully!`);
    ...
    const outBase = outName || entryName;
    const outJs = `./dist/${outBase}.js`;
    const outZip = `./dist/${outBase}.zip`;
    writeFileSync(outJs, worker, 'utf8');

    const zip = new JSZip();
    const zipFile = variant === 'vless' ? 'worker.js' : '_worker.js';
    zip.file(zipFile, worker);
    await zip.generateAsync({
        type: 'nodebuffer',
        compression: 'DEFLATE'
    }).then(nodebuffer => writeFileSync(outZip, nodebuffer));

    console.log(`${success} ${label} Worker → ${outJs}`);
}

async function buildAll() {
    // Build full worker (all protocols)
    await buildWorkerEntry('worker', 'full');
    // Build VLESS-only worker
    await buildWorkerEntry('vless-worker', 'vless', 'vls');
